...
export default function CounsellingModal({
  ...
  className = "",
  children,
  modalTitle = "Book Your",
  modalTitleHighlight = "Free Demo",
  modalSubtitle = "Our team will contact you shortly to schedule your session.",
  leadSource = "",
}: Props) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const sourcePrefix = leadSource.trim() ? `[${leadSource.trim()}] ` : "";
      const res = await fetch(apiUrl("/api/courses/counselling/submit/"), {
        ...
        body: JSON.stringify({
          ...
          message: courseLabel
            ? `${sourcePrefix}Interested course: ${courseLabel}`
            : sourcePrefix.trim(),
          email: form.email,
          phone: form.phone,
          course: isOther
        ...
        ? createPortal(
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                >
                  {modalTitle}{" "}
                  <span className="text-[#0f6ecd]">{modalTitleHighlight}</span>
                </h3>
                <p className="text-sm text-slate-500 mt-2">{modalSubtitle}</p>
              </div>
              <button
                type="button"
